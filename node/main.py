...
from node.core.dependencies import authenticate_node_client
...
database_instance: Database = initialize_resources_and_return_db_context(
...
async def initialize() -> None:
    ...
    logger.warning(
        f"Step 0 (Argument Check) | Detected as {NodeRoles.MASTER.name if parsed_args.prefer_roel is NodeRoles.MASTER.name else NodeRoles.SIDE.name} ..."
    )
    ...
    if parsed_args.prefer_role is NodeRoles.MASTER.name:
        # If this was a master, check first if there are no entries for the master and node.
        master_user_count_stmt = select([func.count()]).where(
            users.c.type == NodeRoles.MASTER.name
        )

        print(master_user_count_stmt)
        count = await database_instance.fetch_val(master_user_count_stmt)

        if not count:
            logger.info("Step 1.2 | Immediately initializing email service...")
    ...
    # This is just an initial log for both NodeRoles.MASTER and NodeRoles.SIDE.
    logger.info(
        f"Step 2.1 | Attempting to look {'for the master node' if parsed_args.prefer_role is NodeRoles.MASTER.name else 'at other nodes'} at host {parsed_args.host}, port {parsed_args.port}..."
    )
...
async def terminate() -> None:
    ...
    await get_blockchain_instance_or_initialize().close()
